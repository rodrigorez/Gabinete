...
- **Hidden Admin Gesture:** Layer DOM transparente (`z-index: 10000`) ativado apenas na região sensível (**canto superior esquerdo**, 15vw × 15vh) para não bloquear a cena 3D. Após **3 toques rápidos** (intervalo < 500ms), o sistema executa `location.replace('adm.html')` — sem entrada no histórico do navegador, impedindo bypass via botão Voltar. A validação de PIN, rate-limiting e lockout ficam inteiramente em `adm.html` via `js/pin-overlay.js`.
